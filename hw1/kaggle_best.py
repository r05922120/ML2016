...
X = np.vstack([np.ones(shape=(1, m)), np.zeros(shape=(162, m))]).T
X = np.matrix(X)
...
result = gradientdescent(X, y, theta, 0.05, 10000,0.1)
finaltheta = result[0]
...
sample.to_csv("kaggle_best.csv", index=False)
